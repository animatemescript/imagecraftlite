...
var users, announcements, insertUserSchema, insertAnnouncementSchema;
...
var DatabaseStorage = class {
  ...
  async getActiveAnnouncement() {
    const { db: db2 } = await Promise.resolve().then(() => (init_db(), db_exports));
    const { eq } = await import("drizzle-orm");
    const [announcement] = await db2.select().from(announcements).where(eq(announcements.isActive, true)).limit(1);
    return announcement || void 0;
  }
  async updateAnnouncement(announcementData) {
    const { db: db2 } = await Promise.resolve().then(() => (init_db(), db_exports));
    await db2.update(announcements).set({ isActive: false });
    const [announcement] = await db2.insert(announcements).values(announcementData).returning();
    return announcement;
  }
};
